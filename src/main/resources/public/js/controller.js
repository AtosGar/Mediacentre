function MediacentreController($scope, template, model, route, $location) {
    ...
    $scope.exportXML_1D = function(){
        model.exportXML_1D(function(result) {
            notify.info('mediacentre.export1D.launched');
        });
    }
    ...
    $scope.getRessources = function (struct) {
       model.getRessources(struct, function(result) {
           ...
           if( result.Erreur != null ) {
               ...
               notify.error(result.Erreur.Message + lang.translate('mediacentre.error.getRessources'));
           } else {
    ...
    $scope.viewDetail = function(resource){
